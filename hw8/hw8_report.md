...
generate_tweet_vector.py begins by scraping Twitter to get the top fifty tweets from each account contained in the accounts.txt file. In order to deal with issues caused by Twitter timeouts and rate limits, it keeps in cache the data from accounts already visited. I ran the file several times to collect all the tweets, as previously visited accounts were pulled from the cache and non-visited accounts were traversed through the scraper. After the tweets are either received from cache or scraped, the words in all of the tweets are extracted and go through filtering. This filtering removes any extraneous characters and URLs, references to other accounts, and words less than 4 characters or greater than 15. All of the words are split by anything non-alphanumeric, which does break up hyphenated words (one of my most common words was 'anti', which I suspect was actually a common prefix to other words). All words are returned as a list in lowercase. Then, the word and its count for that account are stored in a list and returned in conjunction with the account, called _screen_name_ in the code. For each account analyzed, the three files, apcount.txt, sumcounts.txt, and wordcounts.txt, are updated. apcount keeps track of the number of accounts with each word. sumcounts keeps track of the total number of times words are found in all of the accounts, so the total count of the words across tweets. Lastly, wordcounts indicates the number of times words occur in each account. After all of the words have been accounted for, the second stage of filtering for the top 500 words occurs. First, we remove stopwords in the 'fake' way, which removes words that are either too common, in greater than 50% of accounts, or too rare, in less than 10% of accounts. I then created the code to filter for the top 500 most frequent non-stopword terms, which is detailed below. After these top 500 terms are selected, the tweet-term matrix is created. The function that creates the tweet-term matrix takes the top 500 terms that we subset, as well as the dictionary structure with accounts as keys and dictionaries with term/count frequencies as the values. This is the _wordcounts_ dictionary. The matrix is formatted with all of the popular terms, in order, as the header, separated by tabs. Then, each row is added by iteration over the account names in the _wordcounts_ dictionary. For each account, the name is written, and then each term in the list gets cross-referenced with the account's word count (_wc_). If the term was found within the account's tweets and is in the _wc_, then that value is printed, otherwise a zero is printed. Each account is separated by a line, resulting in our tweet-term matrix. 

**B: Explain in detail the code that you added to filter for the 500 most frequent non-stopword terms.**

Considering that wordcounts already contained non-stopword terms, it created a list to hold tuples representing the words and their respective counts. Then, looping through each word in wordcounts, I added the respective count to the tuple and appended it to the list word_and_each_count. I then sorted the word-count pairs in reverse order, to make it easier to grab the top 500, by the count, which was the second value in the tuple and thus index 1. Lastly, I created a loop to iterate through each word in the reversed-ordered list of tuples, grabbing the first 500 words and appending them to the list popularlist.
...
For k = 5, four iterations were needed before the clusters converged. For k = 10 and k = 20, five iterations were needed. However, the first time I ran the clustering, when I didn't set up the output files correctly, k = 5 took six iterations, k = 10 took five iterations and k = 20 only took three iterations. This is evidence of the element of randomness present in k-Means clustering, as each try of clustering will not necessarily result in the same groups because the centroids are randomly generated at the start. Additionally, the algorithm may not always converge before 100 iterations.
