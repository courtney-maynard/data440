...
### Discussion
**Part A:** The URI resulted in the User-Agent request string that is shown above. The different parts of the string indicate information about the computer and browser. This string indicates to websites that the request is coming from a Google Chrome browser on a Mac computer running macOS version 10.15.7. Additionally, *AppleWebKit/537.36* is the browser rendering engine, which is what allows the HTML to be turned into a webpage on our screens. *(KHTML, like Gecko)* indicates that the rendering engine is based on KHTML and behaves like Gecko, while *Chrome/128.0.0.0 Safari/537.36* tells us that the browser is Chrome and has behaviors characteristic of a Safari browser as well. 
